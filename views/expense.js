...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // }
  try {
    const response = await axios.get("http://13.49.64.209:3000/getExpenses?page=1", {
      headers: {
        "Authorization": token,
...
function addexpense(e) {
  ...
  const token = localStorage.getItem('token');
  axios.post("http://13.49.64.209:3000/addexpense", obj, {headers : {"Authorization":token}});
  showonscreen(obj)
...
function showonscreen(e) {
  ...
  if (expenselist) {
    ...
    deletebtn.addEventListener('click', () => {
      const token = localStorage.getItem('token');
      axios.delete(`http://13.49.64.209:3000/deleteExpense/${e.id}`, { headers: { "Authorization": token } });
...
const token = localStorage.getItem('token');
try{
  const response = await axios.get('http://13.49.64.209:3000/purchase/premiummembership', {headers : {"Authorization":token}})
  var options = 
  {
    "key" : response.data.key_id,
    "order_id" : response.data.order.id,
    "handler" : async function (response) {
      const res = await axios.post('http://13.49.64.209:3000/purchase/updatetransactionstatus', {
...
function showleaderboard(){
  ...
  inputElement.onclick = async()=> {
    const token= localStorage.getItem('token')
    const userLeaderboardArray = await axios.get('http://13.49.64.209:3000/premium/showLeaderboard', {headers : {"Authorization":token}})
 ...
 function download(){
  ...
  const token = localStorage.getItem('token');
  try{
    const response = await axios.get('http://13.49.64.209:3000/user/download', { headers: {"Authorization" : token} })
...
async function getDownloads(){
    try {
        ...
        const response = await axios.get('http://13.49.64.209:3000/user/get-downloads', {
            headers: {
...
async function showExpensesForPage(page) {
  ...
  try {
    const response = await axios.get(`http://13.49.64.209:3000/getExpenses?page=${page}`, {
      headers: { Authorization: token ,
        "rows": savedExpensesPerPage}
    });
    const expenseList = document.getElementById('expenseList');
